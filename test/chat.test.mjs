...
test('chat coordinator streams and persists a compatible-provider turn without real credentials', { skip: !process.env.OK_WORKBENCH_INTEGRATION }, async () => {
  ...
  try {
    ...
    const implicitWorkspace = await fetch(`http://127.0.0.1:${port}/api/chat/threads`, { method: 'POST', headers, body: JSON.stringify({ project: 'workspace', provider: 'compatible', model: 'fake-model' }) });
    assert.equal(implicitWorkspace.status, 400); assert.match((await implicitWorkspace.json()).error, /explicit workspace mode/);
    const misplacedWorkspaceMode = await fetch(`http://127.0.0.1:${port}/api/chat/threads`, { method: 'POST', headers, body: JSON.stringify({ project: 'alpha', workspaceMode: true, provider: 'compatible', model: 'fake-model' }) });
    assert.equal(misplacedWorkspaceMode.status, 400); assert.match((await misplacedWorkspaceMode.json()).error, /workspace root/);
    const created = await fetch(`http://127.0.0.1:${port}/api/chat/threads`, { method: 'POST', headers, body: JSON.stringify({ project: 'workspace', workspaceMode: true, provider: 'compatible', model: 'fake-model' }) });
    ...
    const heldCreated = await fetch(`http://127.0.0.1:${port}/api/chat/threads`, { method: 'POST', headers, body: JSON.stringify({ project: 'workspace', workspaceMode: true, provider: 'compatible', model: 'fake-model' }) }); const heldThread = await heldCreated.json();
    ...
    const failedCreated = await fetch(`http://127.0.0.1:${port}/api/chat/threads`, { method: 'POST', headers, body: JSON.stringify({ project: 'workspace', workspaceMode: true, provider: 'compatible', model: 'fake-model' }) }); assert.equal(failedCreated.status, 201); const failedThread = await failedCreated.json();
