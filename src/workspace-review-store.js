...
function defaultRuntime() { return { schemaVersion: SCHEMA_VERSION, attempts: [], retry: {}, invalidReviewStreak: {}, paused: false, nextCheckAt: null, pendingRerun: false, changeDueAt: null, changeDeadlineAt: null, lastJob: null }; }
...
async function writeJson(file, value) { await fs.mkdir(path.dirname(file), { recursive: true, mode: 0o700 }); const temporary = `${file}.${process.pid}.${crypto.randomUUID()}.tmp`; await fs.writeFile(temporary, JSON.stringify(value), { mode: 0o600 }); await fs.rename(temporary, file); }
class WorkspaceReviewStore {
  constructor({ stateDir, workspaceRoot }) { this.root = path.join(stateDir, 'workspace-review', workspaceKey(workspaceRoot)); this.writes = Promise.resolve(); }
  ...
  async serial(work) { const previous = this.writes; let release; this.writes = new Promise(resolve => { release = resolve; }); await previous; try { return await work(); } finally { release(); } }
