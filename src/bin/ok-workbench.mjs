...
async function sandboxCheck() {
  ...
  return ['Sandbox (unsupported platform)', false];
}
async function doctor(args) { const index = args.indexOf('--root'); const root = index >= 0 ? path.resolve(args[index + 1]) : await defaultRoot(); const state = process.env.OK_WORKBENCH_STATE_DIR || process.env.OKF_WORKBENCH_STATE_DIR || path.join(home, '.local', 'state', 'ok-workbench'); const provider = !!(process.env.ANTHROPIC_API_KEY || process.env.OPENAI_API_KEY || process.env.GEMINI_API_KEY || process.env.MISTRAL_API_KEY || process.env.OPENROUTER_API_KEY || process.env.LLM_COMPATIBLE_API_KEY || await fs.access(path.join(state, 'chat', 'pi-agent', 'auth.json')).then(() => true).catch(() => false)); const checks = [['Node >=22.19', Number(process.versions.node.split('.')[0]) >= 22], ['Workspace root', !!(await entries(root))], ['Seed available', !!(await entries(seed))], ['Git', await commandWorks('git', ['--version'])], await sandboxCheck(), ['State outside workspace', !state.startsWith(root + path.sep)], ['Provider configuration present', provider]]; for (const [name, ok] of checks) console.log(`${ok ? 'ok' : 'warn'}  ${name}`); console.log(`root: ${root}`); }
