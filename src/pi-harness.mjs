...
export async function runPiTurn({ provider, model: modelId, effort, messages, projectRoot, workspaceRoot = projectRoot, stateDir, env = process.env, signal, onDelta, onThinking, onTool, onStatus, beforeCreateProject, systemPrompt, noWorkspaceTools = false }) {
  ...
  const tools = noWorkspaceTools ? [] : [
    ...
    defineTool({ name: 'apply_project_update', label: 'Apply OKF project update', description: 'Apply a reviewable batch of project files. Use substantive with a short summary for meaningful project work; its summary is added to log.md and it requires a changed status.md plus a changed index.md for structural additions. Use correction for up to three narrow file corrections. Each new nested directory needs an index.md.', parameters: Type.Object({ kind: Type.Union([Type.Literal('correction'), Type.Literal('substantive')]), summary: Type.Optional(Type.String({ maxLength: 280 })), changes: Type.Array(Type.Object({ path: Type.String(), content: Type.String() }), { minItems: 1, maxItems: 64 }) }), execute: (_id, params) => call('apply_project_update', params) }),
