...
```sh
npm install --global ok-workbench
...
`src/` is authored source. `dist/` is generated by a clean build and is the only application code included in the npm package; `seed/` is the reviewed starter bundle. CI builds, tests, audits package contents, installs the generated tarball, initializes a workspace, runs npm audit, and generates an SBOM.
